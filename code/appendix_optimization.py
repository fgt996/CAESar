...
# Import the models
from models.appendix_optimization import B_CAESar, P_CAESar
...
for theta in [0.05, 0.025, 0.01]: #Iterate over the desired confidence levels
    ...
    for idx_bcv in tqdm(range(N_blocks), desc='Iterating over folds'):
        ...
        for asset in range(N):
            ...
            # solo Barrera
            if not 'B-CAESar' in times[idx_bcv][df.columns[asset]].keys(): # Check if the model has already been computed
                ...
                times[idx_bcv][df.columns[asset]]['B-CAESar'] = time.time()-start # Store the computation time
                predictions[idx_bcv][df.columns[asset]]['B-CAESar'] = res # Store the predictions
                del(mdl); del(res) # Clear the memory

            # solo Patton
            if not 'P-CAESar' in times[idx_bcv][df.columns[asset]].keys(): # Check if the model has already been computed
                start = time.time() # Initialize the timer
                mdl = P_CAESar(theta, 'AS') # Initialize the model
                res = mdl.fit_predict(y, tv, seed=seed, return_train=False) # Fit and predict
                times[idx_bcv][df.columns[asset]]['P-CAESar'] = time.time()-start # Store the computation time
                predictions[idx_bcv][df.columns[asset]]['P-CAESar'] = res # Store the predictions
                del(mdl); del(res) # Clear the memory
...
Assets = df.columns
Algos = ['CAESar', 'B-CAESar', 'P-CAESar']
